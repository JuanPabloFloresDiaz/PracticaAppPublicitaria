    ...
    <div class="container mx-auto px-4">
      ...
      <div class="text-center mb-12">
        ...
        <p class="text-xl text-gray-600 max-w-3xl mx-auto">
          Descubre las últimas novedades y los hitos de nuestras campañas publicitarias,
          mantente informado sobre nuestros logros y próximos eventos.
        </p>
      </div>
      ...
      <div v-else-if="filteredNews && filteredNews.length > 0">
        ...
        <div class="grid md:grid-cols-2 gap-8 max-w-6xl mx-auto mb-8">
          <NewsCard v-for="newsItem in displayedNews" :key="newsItem.id" :news="newsItem" @read-more="handleReadMore" />
        </div>
      ...
      <div v-if="categories.length > 1" class="flex justify-center mt-8">
        <div class="flex flex-wrap gap-2 bg-white p-2 rounded-xl shadow-lg">
          <button @click="selectedCategory = null" class="px-4 py-2 rounded-lg font-medium transition-all duration-200"
          ...
          <button v-for="category in categories" :key="category" @click="selectedCategory = category"
            class="px-4 py-2 rounded-lg font-medium transition-all duration-200" :class="selectedCategory === category
              ? 'bg-purple-600 text-white shadow-md'
              : 'text-gray-700 hover:bg-purple-50 hover:text-purple-600'">
            {{ category }}
          </button>
